...
#------------------------------------------------------------
def mhalo_cut(mh_low,mh_high,mstar,mh,rh,sfr,GroupFirstSub,val_pres,val_dens,val_metal_uw,val_metal_gmw,val_temp_uw,val_temp_gmw,bins):
    idx=np.where((mh > mh_low) & (mh < mh_high))
    mstar,mh,rh,sfr,GroupFirstSub=mstar[idx],mh[idx],rh[idx],sfr[idx],GroupFirstSub[idx]
    nprofs=len(mh)
    val_pres,val_dens,val_metal_uw,val_metal_gmw,val_temp_uw,val_temp_gmw=val_pres[idx,:],val_dens[idx,:],val_metal_uw[idx,:],val_metal_gmw[idx,:],val_temp_uw[idx,:],val_temp_gmw[idx,:]
    val_pres,val_dens,val_metal_uw,val_metal_gmw,val_temp_uw,val_temp_gmw=np.reshape(val_pres,(nprofs,bins)),np.reshape(val_dens,(nprofs,bins)),np.reshape(val_metal_uw,(nprofs,bins)),np.reshape(val_metal_gmw,(nprofs,bins)),np.reshape(val_temp_uw,(nprofs,bins)),np.reshape(val_temp_gmw,(nprofs,bins))
    return mstar,mh,rh,sfr,GroupFirstSub,val_pres,val_dens,nprofs,val_metal_uw,val_metal_gmw,val_temp_uw,val_temp_gmw
...
def extract(simulation,snap): #extract the quantities,adjust as necessary
    stacks=np.load(home+'NPZ_files/profs_IllustrisTNG_'+simulation+'_033_no_emm.npz',allow_pickle=True)
    z              = 0.0 
    val            = stacks['val']
    val_dens=val[0,:]
    val_pres=val[1,:]
    ...
    return z,val_dens,bins,r,val_pres,nprofs,mh,rh,GroupFirstSub,sfr,mstar,val_metal_uw,val_metal_gmw,val_temp_uw,val_temp_gmw

def correct(z,h,mh,mstar,rh,val_dens,val_pres,r,val_temp_uw,val_temp_gmw): #correct all h and comoving factors
    ...
    r /= h
    r /= comoving_factor
    return mh,mstar,rh,val_dens,val_pres,r, val_temp_uw,val_temp_gmw
...
for i in np.arange(len(mh_low_arr)):
    ...
    for j in np.arange(len(simulations)):
        for k in snap:
            z,val_dens,bins,r,val_pres,nprofs,mh,rh,GroupFirstSub,sfr,mstar,val_metal_uw,val_metal_gmw,val_temp_uw,val_temp_gmw=extract(simulations[j],k)
            omegab=0.049
            h=0.6711
            ...
            mh,mstar,rh,val_dens,val_pres,r,val_temp_uw,val_temp_gmw=correct(z,h,mh,mstar,rh,val_dens,val_pres,r,val_temp_uw,val_temp_gmw)
    
            if mh_cut==True:
                    mstar,mh,rh,sfr,GroupFirstSub,val_pres,val_dens,nprofs,val_metal_uw,val_metal_gmw,val_temp_uw,val_temp_gmw=mhalo_cut(mh_low,mh_high,mstar,mh,rh,sfr,GroupFirstSub,val_pres,val_dens,val_metal_uw,val_metal_gmw,val_temp_uw,val_temp_gmw,bins)
    
            r_mpc=r/1.e3
        

            masses.append(np.mean(mh))
            mean_r200c_mpc=np.mean(rh/1.e3)
            ...
            r_mpc_cut,val_dens=outer_cut_multi(7,r_mpc,val_dens)
            r_mpc_cut2,val_dens=inner_cut_multi(3.e-4,r_mpc_cut,val_dens)
            r_mpc_cut,val_pres=outer_cut_multi(7,r_mpc,val_pres)
            r_mpc_cut2,val_pres=inner_cut_multi(3.e-4,r_mpc_cut,val_pres)
            r_mpc_cut,val_metal_uw=outer_cut_multi(7,r_mpc,val_metal_uw)
            r_mpc_cut2,val_metal_uw=inner_cut_multi(3.e-4,r_mpc_cut,val_metal_uw)
            r_mpc_cut,val_metal_gmw=outer_cut_multi(7,r_mpc,val_metal_gmw)
            r_mpc_cut2,val_metal_gmw=inner_cut_multi(3.e-4,r_mpc_cut,val_metal_gmw)
            r_mpc_cut,val_temp_uw=outer_cut_multi(7,r_mpc,val_temp_uw)
            r_mpc_cut2,val_temp_uw=inner_cut_multi(3.e-4,r_mpc_cut,val_temp_uw)
            r_mpc_cut,val_temp_gmw=outer_cut_multi(7,r_mpc,val_temp_gmw)
            ...
            x=r_mpc_cut2/mean_r200c_mpc
            ax1.loglog(x,mean_unnorm_dens,color=colors[j],label='%s nhalo:%i'%(simulations[j],nprofs))
            ax2.loglog(x,mean_unnorm_pres,color=colors[j],label='%s'%str(ASN1[j]))
            ax3.loglog(x,mean_unnorm_metal_uw,color=colors[j])#,label='uw')
            ax3.loglog(x,mean_unnorm_metal_gmw,color=colors[j],linestyle='dashed')#,label='gmw')
            ax4.loglog(x,mean_unnorm_temp_uw,color=colors[j])#,label='uw')
            ax4.loglog(x,mean_unnorm_temp_gmw,color=colors[j],linestyle='dashed')#,label='gmw')
...
ax3.set_xlabel(r'$R/R_{200c}$',size=14)
ax4.set_xlabel(r'$R/R_{200c}$',size=14)
...
plt.savefig(home+'Figures/vary_ASN1_11-12.png')
